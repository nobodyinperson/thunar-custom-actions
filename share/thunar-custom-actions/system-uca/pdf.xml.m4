...
<action>
	...
	<command>TUCA_INIT();TUCA_WAIT(TUCA_CONVERT() TUCA_ALL_FILES() -gravity Center -background white -resize 1654x2338 -extent 1654x2338 -units PixelsPerInch -density 200x200 TUCA_FIRST_FILE().pdf,$(eval_gettext 'converting to PDF...'))</command>
...
<action>
	...
	<command>TUCA_INIT();TUCA_WAIT(TUCA_GS() -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=TUCA_FIRST_FILE()-all.pdf TUCA_ALL_FILES(),$(eval_gettext 'concatenating PDFs...'))</command>
...
<action>
	...
	<command>TUCA_INIT();TUCA_LOOP(TUCA_LOOP_PB_TEXT($(eval_gettext 'optimizing PDF... $TUCA_FILE_BASE_VAR()'));TUCA_GS() -dBATCH -dNOPAUSE -q -dPDFSETTINGS=/screen -sDEVICE=pdfwrite -dDownsampleColorImages=true -dDownsampleGrayImages=true -dDownsampleMonoImages=true -dColorImageResolution=72 -dGrayImageResolution=72 -dMonoImageResolution=72 -sOutputFile=TUCA_TEMP() TUCA_FILE())</command>
